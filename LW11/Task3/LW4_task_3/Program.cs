...
using LW4_task_3.Bridge.Factory;
...
builder.Services.AddScoped<ExpectationMessage>();
builder.Services.AddScoped<ActiveMessage>();
builder.Services.AddScoped<OverdueMessage>();
builder.Services.AddScoped<ArchivedMessage>();

string send = builder.Configuration.GetValue<string>("SendTo");
builder.Services.AddScoped<IMessageSender>(provider =>
{
    switch (send)
    {
        case "File": return new FileSender();
        default:
            {
                var repository = provider.GetRequiredService<IMessageRepository>();
                return new DBSender(repository);
            }
    }
});

builder.Services.AddScoped<MessageFactoryService>();
